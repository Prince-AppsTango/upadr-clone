...
@Composable
fun PrepProcessOverviewDetailsScreen(drawerState:DrawerState,navController: NavController) {
    SafeArea {
        Column(
            modifier = Modifier
            ...
            TopDrawerNavigation(elevation = 8,drawerState,navController)
            Column(
                modifier = Modifier
                    .fillMaxSize(1f)
                    .padding(20.dp,10.dp)
            ) {
                Title(text = "Step 2", color = Black, fontSize = 20)
            }
        }
    }
}